...
// 当前日期（还未到调价时间）
const currentDate = '2026-03-08'; // 使用 3-8 作为当前日期，因为还未到 3-9 24:00

// 读取当前价格数据（2026-02-24 调价后的价格）
...
dateRange.forEach(date => {
  for (const province in provinceCityMap) {
    ...
    // 添加一些随机波动（±0.01 元，模拟市场小幅波动）
    const randomFluctuation = () => (Math.random() - 0.5) * 0.02;

    // 价格保持稳定，只有小幅波动
    const basePrice = currentPrices[province];

    const dailyGas92 = parseFloat((basePrice.gas92 + randomFluctuation()).toFixed(2));
    const dailyGas95 = parseFloat((basePrice.gas95 + randomFluctuation()).toFixed(2));
    const dailyGas98 = parseFloat((basePrice.gas98 + randomFluctuation()).toFixed(2));
    const dailyDiesel0 = parseFloat((basePrice.diesel0 + randomFluctuation()).toFixed(2));
...
console.log('   上海当前价格:', currentPrices['上海市']);
